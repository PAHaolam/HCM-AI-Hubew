...
model_xlm, _, preprocess_xlm = open_clip.create_model_and_transforms(
    'xlm-roberta-large-ViT-H-14', pretrained='frozen_laion5b_s13b_b90k')
model_siglip, _, preprocess_siglip = open_clip.create_model_and_transforms(
...
for model_name in id2imgfiles:
    for key in id2imgfiles[model_name]:
        id2imgfiles[model_name][key] = os.path.join(
            *id2imgfiles[model_name][key].split('/')[-2:])
        ...
        if images:
            ...
            for file_name in images:
                full_path = os.path.join(
                    IMAGE_FOLDER, file_name.replace('/', os.sep))
